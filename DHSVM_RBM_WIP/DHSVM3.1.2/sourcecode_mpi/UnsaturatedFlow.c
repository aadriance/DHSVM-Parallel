...
void UnsaturatedFlow(int Dt, float DX, float DY, float Infiltration,
                     ...
                     float BankHeight, float *TableDepth, float *Runoff,
                     float *Moist, int RoadRouteOption, int InfiltOption,
                     float *RoadIExcess) {
  ...
  /* first take care of infiltration through the roadbed/channel, then through
  ...
  if (*TableDepth <= BankHeight) { /* watertable above road/channel surface */
    ...
    if (CutBankZone == NSoilLayers) {
      Moist[NSoilLayers] +=
          RoadbedInfiltration / (DeepLayerDepth * Adjust[NSoilLayers]);
    } else if (CutBankZone >= 0) {
      Moist[CutBankZone] +=
          RoadbedInfiltration / (RootDepth[CutBankZone] * Adjust[CutBankZone]);
    }
  }
  ...
  if (Moist[NSoilLayers] < FCap[NSoilLayers - 1]) {
    ...
    /*  printf("Warning: Deep layer soil moisture is less than field
     * capacity.\n"); */
