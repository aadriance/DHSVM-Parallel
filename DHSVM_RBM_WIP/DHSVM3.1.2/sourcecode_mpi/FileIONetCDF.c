 ...
 * Modified was made to Read2DMatrix by Ning (2013)
   ...
   Comment     :First, make sure that the input NETCDF file to Read2DMatrix
                is in 3 dimensions. In this case, the 1st dimention should be
 time, the 2nd is y (lat), and 3rd is x (lon). Using the command 'ncdump
 -in.nc', we can see that the descending/ascending order in which the NetCDF
 file uses to stores coordinates. The order is important b/c it decides in which
 order the NetCDF files is read out and stored in a 2D matrix. Simply put, in
 the input netcdf file, if the first variable value corresponds to the smallest
 x and y, which is the lower left corner in the spatial sense, then it shouldn't
 be assigned to Matrix[0][0] which is in the upper left corner. Instead it
 should be stored in lower left corner. So the Read2DMatrix function is changed
 so that it reads x and y in .nc input file, and output a flag that will be used
 as an indicator of whethe or not the matrix should be reversed. Note that this
 program now handles two cases: 1) .nc generated from gdal tool, in which x and
 y are both in an ascending order; 2) .nc generated from arcmap, in which x is
 in an ascending and y in an descedning order, and no reverse needs to be made
 in this case. Just be aware that a time dimention has to be added to arc
 generated .nc file. (check the tutorial for instructions). The matrix will be
 reversed in the 1st case in the program that calls Read2DMATRIX (Ning, Feb
 2013)
 */
...
void CreateMapFileNetCDF(char *FileName, ...) {
  ...
  nc_check_err(ncstatus, __LINE__, __FILE__);
  ncstatus =
      nc_put_att_text(ncid, varidtime, ATT_NAME, strlen(TIME_DIM), TIME_DIM);
  nc_check_err(ncstatus, __LINE__, __FILE__);
  ncstatus = nc_put_att_text(ncid, varidtime, ATT_LONGNAME, strlen(TIME_DIM),
                             TIME_DIM);
  nc_check_err(ncstatus, __LINE__, __FILE__);
  ncstatus =
      nc_put_att_text(ncid, varidtime, ATT_UNITS, strlen("index"), "index");
  nc_check_err(ncstatus, __LINE__, __FILE__);
  /*   ncstatus = nc_put_att_text(ncid, varidtime, ATT_FORMAT, strlen("%g"),
   * "%g"); */
  /*   nc_check_err(ncstatus, __LINE__, __FILE__); */
  ...
  nc_check_err(ncstatus, __LINE__, __FILE__);
  ncstatus = nc_put_att_text(ncid, varidnorth, ATT_LONGNAME, strlen("Northing"),
                             "Northing");
  nc_check_err(ncstatus, __LINE__, __FILE__);
  ncstatus = nc_put_att_text(ncid, varidnorth, ATT_UNITS, strlen("m"), "m");
  nc_check_err(ncstatus, __LINE__, __FILE__);
  /*   ncstatus = nc_put_att_text(ncid, varidnorth, ATT_FORMAT, strlen("%g"),
   * "%g"); */
  /*   nc_check_err(ncstatus, __LINE__, __FILE__); */
  ...
  nc_check_err(ncstatus, __LINE__, __FILE__);
  /*   ncstatus = nc_put_att_text(ncid, varideast, ATT_FORMAT, strlen("%g"),
   * "%g"); */
  /*   nc_check_err(ncstatus, __LINE__, __FILE__); */
...
int Read2DMatrixNetCDF(char *FileName, void *Matrix, int NumberType, int NY,
  ...
  assigned to the lower left corner cell that has lowest X (lon) & Y (lat)
  value. (see more comments in the header of this C file). */
  LatisAsc = 1;
...
int Write2DMatrixNetCDF(char *FileName, void *Matrix, int NumberType, int NY,
  ...
  if (ncstatus == NC_ENOTVAR) { /* Variable not defined */
    ...
    nc_check_err(ncstatus, __LINE__, __FILE__);
    ncstatus =
        nc_def_var(ncid, DMap->Name, DMap->NumberType, 3, dimids, &varid);
    nc_check_err(ncstatus, __LINE__, __FILE__);

    /* write variable attributes */
    ncstatus =
        nc_put_att_text(ncid, varid, ATT_NAME, strlen(DMap->Name), DMap->Name);
    nc_check_err(ncstatus, __LINE__, __FILE__);
...
static int ncUpdateGlobalHistory(int argc, char **argv, int ncid) {
  ...
  if (status == NC_ENOTATT) { /* no history attibrute present */
    ...
    status =
        nc_put_att_text(ncid, NC_GLOBAL, ATT_HISTORY, strlen(update), update);
...
gcc -Wall -g -o test_netcdf -DTEST_FILEIONETCDF FileIONetCDF.c ReportError.c
Files.c SizeOfNT.c VarID.c -lnetcdf then run the program by typing test_netcdf
*******************************************************************************/
...
int main(void) {
  ...
  Read2DMatrixNetCDF(DMap.FileName, (void *)ReadArray, DMap.NumberType, Map.NY,
                     Map.NX, 0, DMap.Name);
