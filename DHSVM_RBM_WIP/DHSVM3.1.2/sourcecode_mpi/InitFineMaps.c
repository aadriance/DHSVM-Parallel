 ...
 * ORIG-DATE:    Oct-03
 * Last Change:  Mon Oct 27 14:40:44 2003 by Colleen O. Doten
 * <colleen@hydro.washington.edu> DESCRIPTION:  Initialize terrain coverages for
 * fine resolution map DESCRIP-END. FUNCTIONS:    InitFineMaps() COMMENTS:
 */
...
void InitFineMaps(LISTPTR Input, OPTIONSTRUCT *Options, MAPSIZE *Map,
  ...
  STRINIENTRY StrEnv[] = {{"FINEDEM", "DEM FILE", "", ""},
                          {"FINEDEM", "MASK FILE", "", ""},
                          {NULL, NULL, "", NULL}};
  ...
  Read2DMatrix(StrEnv[demfile].VarStr, Elev, NumberType, Map->NYfine,
               Map->NXfine, 0, VarName);
  ...
  if (IsEmptyStr(StrEnv[1].VarStr)) {
    ...
    if (!(Mask = (unsigned char *)calloc(Map->NXfine * Map->NYfine,
                                         SizeOfNumberType(NumberType))))
      ReportError((char *)Routine, 1);
    Read2DMatrix(StrEnv[maskfile].VarStr, Mask, NumberType, Map->NYfine,
                 Map->NXfine, 0, VarName);
    MASKFLAG = TRUE;
  }
      ...
      for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
        for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
          if (INBASIN((*TopoMap)[y][x].Mask)) {
            ...
            // This means that failures may still try to leave the basin if the
            // coarse mask isn't wide enough because some of the drainage area
            // may be cropped.
            /*       if (!INBASIN((*TopoMap)[y][x].Mask)) */
            /* 		(*(*FineMap)[yy][xx]).Mask = (*TopoMap)[y][x].Mask; */
            /*    } */
            (*(*FineMap)[yy][xx]).bedrock =
                (*(*FineMap)[yy][xx]).Dem - (*SoilMap)[y][x].Depth;
  ...
  /* NumCellsfine is used in CalcTopoIndex.  The topo index is calculated for
     every fine cell within the boundary of the coarse mask, so this number may
     exceed the number of pixels within the fine resolution mask.  */
  ...
  for (y = 0; y < Map->NY; y++) {
    for (x = 0; x < Map->NX; x++) {
      if (INBASIN((*TopoMap)[y][x].Mask)) {
        if (!((*TopoMap)[y][x].OrderedTopoIndex =
                  (ITEM *)calloc(Map->NumFineIn, sizeof(ITEM))))
          ReportError((char *)Routine, 1);
      }
    }
  }
      ...
      if (INBASIN((*TopoMap)[y][x].Mask)) {
        ...
        for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
          for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
            yy = (int)y * Map->DY / Map->DMASS + ii;
            xx = (int)x * Map->DX / Map->DMASS + jj;
            (*TopoMap)[y][x].OrderedTopoIndex[k].Rank =
                (*(*FineMap)[yy][xx]).TopoIndex;
