...
void InitMetMaps(int NDaySteps, MAPSIZE *Map, MAPSIZE *Radar,
                 ...
                 RADARPIX ***RadarMap, RADCLASSPIX ***RadMap, SOILPIX **SoilMap,
  ...
  for (y = 0; y < Map->NY; y++) {
    for (x = 0; x < Map->NX; x++) {
      if (INBASIN(TopoMap[y][x].Mask)) {
        ...
        if (!((*EvapMap)[y][x].EPot = (float *)calloc(NVeg + 1, sizeof(float))))
          ReportError((char *)Routine, 1);

        if (!((*EvapMap)[y][x].EAct = (float *)calloc(NVeg + 1, sizeof(float))))
          ReportError((char *)Routine, 1);

        if (!((*EvapMap)[y][x].EInt = (float *)calloc(NVeg, sizeof(float))))
          ReportError((char *)Routine, 1);

        if (!((*EvapMap)[y][x].ESoil = (float **)calloc(NVeg, sizeof(float *))))
          ReportError((char *)Routine, 1);
...
void InitShadeMap(OPTIONSTRUCT *Options, int NDaySteps, int NY, int NX,
  ...
  if (!(Array = (float *)calloc(NY * NX, sizeof(float))))
    ReportError((char *)Routine, 1);
  Read2DMatrix(Options->SkyViewDataPath, Array, NumberType, NY, NX, 0, VarName,
               0);
  for (y = 0; y < NY; y++) {
    for (x = 0; x < NX; x++) {
