...
void InitTables(int StepsPerDay, LISTPTR Input, OPTIONSTRUCT *Options,
                SOILTABLE **SType, LAYER *Soil, VEGTABLE **VType, LAYER *Veg,
                SNOWTABLE **SnowAlbedo) {
...
/********************************************************************************
  ...
  Comments     :
********************************************************************************/
int InitSoilTable(OPTIONSTRUCT *Options, SOILTABLE **SType, LISTPTR Input,
                  LAYER *Soil, int InfiltOption) {
  const char *Routine = "InitSoilTable";
  int i;      /* counter */
  int j;      /* counter */
  int NSoils; /* Number of soil types */
  char KeyName[thermal_capacity + 1][BUFSIZE + 1];
  char *KeyStr[] = {"SOIL DESCRIPTION",       "LATERAL CONDUCTIVITY",
                    "EXPONENTIAL DECREASE",   "DEPTH THRESHOLD",
                    "MAXIMUM INFILTRATION",   "CAPILLARY DRIVE",
                    "SURFACE ALBEDO",         "MANNINGS N",
                    "NUMBER OF SOIL LAYERS",  "POROSITY",
                    "PORE SIZE DISTRIBUTION", "BUBBLING PRESSURE",
                    "FIELD CAPACITY",         "WILTING POINT",
                    "BULK DENSITY",           "VERTICAL CONDUCTIVITY",
  ...
  for (i = 0; i < NSoils; i++) {
    ...
    if (!((*SType)[i].Porosity =
              (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    if (!((*SType)[i].PoreDist =
              (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    ...
    if (!((*SType)[i].FCap =
              (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    if (!((*SType)[i].WP = (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    if (!((*SType)[i].Dens =
              (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    if (!((*SType)[i].Ks = (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    if (!((*SType)[i].KhDry =
              (float *)calloc((*SType)[i].NLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
...
int InitVegTable(VEGTABLE **VType, LISTPTR Input, OPTIONSTRUCT *Options,
  ...
  char *KeyStr[] = {"VEGETATION DESCRIPTION",
                    "OVERSTORY PRESENT",
                    "UNDERSTORY PRESENT",
                    "FRACTIONAL COVERAGE",
                    "HEMI FRACT COVERAGE",
  ...
  for (i = 0; i < NVegs; i++) {
    ...
    if (!((*VType)[i].Fract =
              (float *)calloc((*VType)[i].NVegLayers, sizeof(float))))
      ReportError((char *)Routine, 1);

    if (Options->CanopyRadAtt == VARIABLE) {
      if (!((*VType)[i].HemiFract =
                (float *)calloc((*VType)[i].NVegLayers, sizeof(float))))
        ReportError((char *)Routine, 1);
    } else {
    ...
    if (!((*VType)[i].Rpc =
      ...
      ReportError((char *)Routine, 1);
    if (!((*VType)[i].Albedo =
              (float *)calloc(((*VType)[i].NVegLayers + 1), sizeof(float))))
      ReportError((char *)Routine, 1);
    if (!((*VType)[i].MaxInt =
              (float *)calloc((*VType)[i].NVegLayers, sizeof(float))))
      ReportError((char *)Routine, 1);
    ...
    if (!((*VType)[i].RootDepth =
              (float *)calloc((*VType)[i].NSoilLayers, sizeof(float))))
      ReportError((char *)Routine, 1);

    if (!((*VType)[i].LAIMonthly =
              (float **)calloc((*VType)[i].NVegLayers, sizeof(float *))))
      ReportError((char *)Routine, 1);
    for (j = 0; j < (*VType)[i].NVegLayers; j++) {
...
void InitSnowTable(SNOWTABLE **SnowAlbedo, int StepsPerDay) {
  ...
  if (!(*SnowAlbedo = (SNOWTABLE *)calloc((int)((DAYPYEAR + 1) * StepsPerDay),
                                          sizeof(SNOWTABLE))))
    ReportError((char *)Routine, 1);
