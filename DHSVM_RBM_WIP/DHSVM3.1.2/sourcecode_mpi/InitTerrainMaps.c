...
void InitTopoMap(LISTPTR Input, OPTIONSTRUCT *Options, MAPSIZE *Map,
  ...
  STRINIENTRY StrEnv[] = {{"TERRAIN", "DEM FILE", "", ""},
  ...
  if (!(Elev =
            (float *)calloc(Map->NX * Map->NY, SizeOfNumberType(NumberType))))
    ReportError((char *)Routine, 1);

  flag = Read2DMatrix(StrEnv[demfile].VarStr, Elev, NumberType, Map->NY,
                      Map->NX, 0, VarName, 0);
  ...
  if (!(Mask = (unsigned char *)calloc(Map->NX * Map->NY,
    ...
    ReportError((char *)Routine, 1);
  flag = Read2DMatrix(StrEnv[maskfile].VarStr, Mask, NumberType, Map->NY,
                      Map->NX, 0, VarName, 0);
...
void InitSoilMap(LISTPTR Input, OPTIONSTRUCT *Options, MAPSIZE *Map,
  ...
  int flag;
  STRINIENTRY StrEnv[] = {{"SOILS", "SOIL MAP FILE", "", ""},
  ...
  for (y = 0, i = 0; y < Map->NY; y++) {
    for (x = 0; x < Map->NX; x++, i++) {
      ...
      if (INBASIN(TopoMap[y][x].Mask)) {
        if (!((*SoilMap)[y][x].Moist = (float *)calloc(
                  (Soil->NLayers[Type[i] - 1] + 1), sizeof(float))))
          ReportError((char *)Routine, 1);
        if (!((*SoilMap)[y][x].Perc =
...
void InitVegMap(OPTIONSTRUCT *Options, LISTPTR Input, MAPSIZE *Map,
  ...
  if (!(Type = (unsigned char *)calloc(Map->NX * Map->NY,
                                       SizeOfNumberType(NumberType))))
    ReportError((char *)Routine, 1);
  flag = Read2DMatrix(VegMapFileName, Type, NumberType, Map->NY, Map->NX, 0,
                      VarName, 0);
