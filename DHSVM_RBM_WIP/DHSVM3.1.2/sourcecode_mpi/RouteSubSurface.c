...
/*****************************************************************************
  ...
  WORK IN PROGRESS
*****************************************************************************/
void RouteSubSurface(int Dt, MAPSIZE *Map, TOPOPIX **TopoMap, VEGTABLE *VType,
                     VEGPIX **VegMap, ROADSTRUCT **Network, SOILTABLE *SType,
                     SOILPIX **SoilMap, CHANNEL *ChannelData, TIMESTRUCT *Time,
                     OPTIONSTRUCT *Options, char *DumpPath, SEDPIX **SedMap,
      ...
      if (INBASIN(TopoMap[y][x].Mask)) {
        ...
        if (!channel_grid_has_channel(ChannelData->stream_map, x, y)) {
          ...
          if (SoilMap[y][x].TableDepth < SoilMap[y][x].Depth) {
            ...
            Transmissivity = CalcTransmissivity(
                SoilMap[y][x].Depth, depth, SType[SoilMap[y][x].Soil - 1].KsLat,
            ...
            AvailableWater = CalcAvailableWater(
                ...
                SType[SoilMap[y][x].Soil - 1].FCap, SoilMap[y][x].TableDepth,
                Adjust);
