...
void ExecDump(MAPSIZE *Map, DATE *Current, DATE *Start, OPTIONSTRUCT *Options,
              ...
              PIXRAD **RadiationMap, PRECIPPIX **PrecipMap,
  ...
  if (Options->Sediment)
    DumpPixSed(Current, IsEqualTime(Current, Start), &(Dump->AggregateSediment),
               &(Total->Sediment), &(Total->Road),
               Total->SedimentOverlandInflow, Total->SedimentOverroadInflow,
               &(Total->Fine));
    ...
    for (i = 0; i < Dump->NPix; i++) {
      ...
      if (Options->MassWaste) {
        ...
        for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
          for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
            ...
            PixAggFineMap.SedimentToChannel +=
                (*FineMap[yy][xx]).SedimentToChannel;
          }
        }
      ...
      if (Options->InitSedFlag &&
        ...
        for (i = 0; i < NSEDSIZES; i++) {
          overlandinflow += ChannelData->stream_map[x][y]
                                ->channel->sediment.overlandinflow[i];
        }
      } else
        overlandinflow = -999.;
      ...
      if (Options->RoadRouting &&
        ...
        for (i = 0; i < NSEDSIZES; i++) {
          overroadinflow +=
              ChannelData->road_map[x][y]->channel->sediment.overroadinflow[i];
        }
      } else
        overroadinflow = -999.;
      ...
      if (Options->Sediment)
        DumpPixSed(Current, IsEqualTime(Current, Start),
                   &(Dump->Pix[i].OutFileSediment), &(SedMap[y][x]),
    ...
    for (i = 0; i < Dump->NMaps; i++) {
      for (j = 0; j < Dump->DMap[i].N; j++) {
        if (IsEqualTime(Current, &(Dump->DMap[i].DumpDate[j]))) {
          ...
          fprintf(stdout, "\n");
          DumpMap(Map, Current, &(Dump->DMap[i]), TopoMap, EvapMap, PrecipMap,
                  RadiationMap, SnowMap, SoilMap, SedMap, FineMap, Soil, VegMap,
                  Veg, Network, Options);
        }
      }
    }
...
void DumpMap(MAPSIZE *Map, DATE *Current, MAPDUMP *DMap, TOPOPIX **TopoMap,
  ...
  if (DMap->ID >= 800 && DMap->ID < 900) {
    numPoints =
        Map->NX * Map->NY * Map->DX * Map->DY / (Map->DMASS * Map->DMASS);
  } else {
      ...
      for (y = 0; y < Map->NY; y++) {
        for (x = 0; x < Map->NX; x++) {
          if (INBASIN(TopoMap[y][x].Mask)) {
            ...
            if (DMap->Layer > Veg->MaxLayers)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((EvapMap[y][x].EPot[NVeg] - Offset) / Range *
                                  MAXUCHAR);
            else if (DMap->Layer <= NVeg)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((EvapMap[y][x].EPot[DMap->Layer - 1] -
                                   Offset) /
                                  Range * MAXUCHAR);
            else
      ...
      for (y = 0; y < Map->NY; y++) {
        for (x = 0; x < Map->NX; x++) {
          if (INBASIN(TopoMap[y][x].Mask)) {
            ...
            if (DMap->Layer > Veg->MaxLayers)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((EvapMap[y][x].EInt[NVeg] - Offset) / Range *
                                  MAXUCHAR);
            else if (DMap->Layer <= NVeg)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((EvapMap[y][x].EInt[DMap->Layer - 1] -
                                   Offset) /
                                  Range * MAXUCHAR);
            else
              ((unsigned char *)Array)[y * Map->NX + x] = 0;
      ...
      for (y = 0; y < Map->NY; y++) {
        for (x = 0; x < Map->NX; x++) {
          if (INBASIN(TopoMap[y][x].Mask)) {
            ...
            if (DMap->Layer > NVeg)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((EvapMap[y][x].EAct[NVeg] - Offset) / Range *
                                  MAXUCHAR);
            else if (DMap->Layer <= NVeg)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((EvapMap[y][x].EAct[DMap->Layer - 1] -
                                   Offset) /
                                  Range * MAXUCHAR);
            else
              ((unsigned char *)Array)[y * Map->NX + x] = 0;
  ...
  case 201:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((PrecipMap[y][x].Precip - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 301:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((RadMap[y][x].ObsShortIn - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 302:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((RadMap[y][x].RBMNetShort - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 303:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((RadMap[y][x].PixelBeam - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 406:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SnowMap[y][x].PackWater - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 407:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SnowMap[y][x].TPack - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 408:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SnowMap[y][x].SurfWater - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 409:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SnowMap[y][x].TSurf - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 410:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SnowMap[y][x].ColdContent - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
      ...
      for (y = 0; y < Map->NY; y++) {
        for (x = 0; x < Map->NX; x++) {
          if (INBASIN(TopoMap[y][x].Mask)) {
            ...
            if (DMap->Layer <= NSoil)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((SoilMap[y][x].Moist[DMap->Layer - 1] -
                                   Offset) /
                                  Range * MAXUCHAR);
            else
              ((unsigned char *)Array)[y * Map->NX + x] = 0;
      ...
      for (y = 0; y < Map->NY; y++) {
        for (x = 0; x < Map->NX; x++) {
          if (INBASIN(TopoMap[y][x].Mask)) {
            ...
            if (DMap->Layer <= NSoil)
              ((unsigned char *)Array)[y * Map->NX + x] =
                  (unsigned char)((SoilMap[y][x].Perc[DMap->Layer - 1] -
                                   Offset) /
                                  Range * MAXUCHAR);
            else
              ((unsigned char *)Array)[y * Map->NX + x] = 0;
  ...
  case 503:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SoilMap[y][x].TableDepth - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 504:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SoilMap[y][x].SatFlow - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 505:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SoilMap[y][x].TSurf - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 513:
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SoilMap[y][x].IExcess - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 514:
    ...
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SoilMap[y][x].InfiltAcc - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 801:
    ...
    if (DMap->Resolution == MAP_OUTPUT) {
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
              for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
                ...
                xx = (int)x * Map->DX / Map->DMASS + jj;
                ((float *)
                     Array)[yy * (int)(Map->NX * Map->DX / Map->DMASS) + xx] =
                    (*FineMap[yy][xx]).Dem;
              }
            }
          } else {
      ...
      Write2DMatrix(DMap->FileName, Array, DMap->NumberType,
                    (int)(Map->NY * Map->DY / Map->DMASS),
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
              for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
                ...
                ((unsigned char *)
                     Array)[yy * (int)(Map->NX * Map->DX / Map->DMASS) + xx] =
                    (unsigned char)(((*FineMap[yy][xx]).Dem - Offset) / Range *
                                    MAXUCHAR);
              }
            }
          } else {
      ...
      Write2DMatrix(DMap->FileName, Array, NC_BYTE,
                    (int)(Map->NY * Map->DY / Map->DMASS),
                    (int)(Map->NX * Map->DX / Map->DMASS), DMap, Index);
    } else
      ReportError(VarIDStr, 66);
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
              for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
                ...
                ((unsigned char *)
                     Array)[yy * (int)(Map->NX * Map->DX / Map->DMASS) + xx] =
                    (unsigned char)(((*FineMap[yy][xx]).SatThickness - Offset) /
                                    Range * MAXUCHAR);
              }
            }
          } else {
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
              for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
                ...
                ((unsigned char *)
                     Array)[yy * (int)(Map->NX * Map->DX / Map->DMASS) + xx] =
                    (unsigned char)(((*FineMap[yy][xx]).DeltaDepth - Offset) /
                                    Range * MAXUCHAR);
              }
            }
          } else {
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
              for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
                ...
                ((unsigned char *)
                     Array)[yy * (int)(Map->NX * Map->DX / Map->DMASS) + xx] =
                    (unsigned char)(((*FineMap[yy][xx]).Probability - Offset) /
                                    Range * MAXUCHAR);
              }
            }
          } else {
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
              for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
                ...
                ((unsigned char *)
                     Array)[yy * (int)(Map->NX * Map->DX / Map->DMASS) + xx] =
                    (unsigned char)(((*FineMap[yy][xx]).SedimentToChannel -
                                     Offset) /
                                    Range * MAXUCHAR);
              }
            }
  ...
  case 901:
    ...
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SedMap[y][x].SedFluxOut - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 902:
    ...
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((SedMap[y][x].Erosion - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
  ...
  case 903:
    ...
    if (DMap->Resolution == MAP_OUTPUT) {
      ...
      for (y = 0; y < Map->NY; y++)
        for (x = 0; x < Map->NX; x++)
          ((unsigned char *)Array)[y * Map->NX + x] =
              (unsigned char)((Network[y][x].Erosion - Offset) / Range *
                              MAXUCHAR);
      Write2DMatrix(DMap->FileName, Array, NC_BYTE, Map->NY, Map->NX, DMap,
                    Index);
    } else
...
void DumpPix(DATE *Current, int first, FILES *OutFile, EVAPPIX *Evap,
             PRECIPPIX *Precip, RADCLASSPIX *Rad, SNOWPIX *Snow, SOILPIX *Soil,
  ...
  if (first == 1) {
    ...
    fprintf(OutFile->FilePtr,
            "HasSnow SnowCover LastSnow    Swq       Melt   ");
    ...
    fprintf(OutFile->FilePtr,
            "  TableDepth   SatFlow   Runoff    IMP-DS    IExcess  ");
  ...
  /* Snow */
  fprintf(OutFile->FilePtr, " %1d %1d %4d %g %g %g %g", Snow->HasSnow,
          Snow->SnowCoverOver, Snow->LastSnow, Snow->Swq, Snow->Melt,
          Snow->PackWater, Snow->TPack);
  ...
  for (i = 0; i < NVeg + 1; i++)
    fprintf(OutFile->FilePtr, " %g",
            Evap->EPot[i]); /* Potential transpiration */
  for (i = 0; i < NVeg + 1; i++)
  ...
  for (i = 0; i < NVeg; i++)
    for (j = 0; j < NSoil; j++)
      fprintf(OutFile->FilePtr, " %g",
              Evap->ESoil[i][j]); /*transpiration from each veg layer*/
  ...
  fprintf(OutFile->FilePtr, " %g %g %g %g %g %g %g", Soil->TSurf, Soil->Qnet,
          Soil->Qs, Soil->Qe, Soil->Qg, Soil->Qst, Soil->Ra);
...
void DumpPixSed(DATE *Current, int first, FILES *OutFileSediment,
  ...
  if (first == 1) {
    ...
    fprintf(OutFileSediment->FilePtr, "SatThick DeltaDepth Probability "
                                      "TotMassWasting TotMassDeposition "
                                      "TotSedToChannel ");
    fprintf(OutFileSediment->FilePtr, "Erosion SedFluxOut OverlandInflow ");
    fprintf(OutFileSediment->FilePtr,
            "RdErosion RdSedToHill OverroadInflow \n");
  }
  ...
  fprintf(OutFileSediment->FilePtr, " %g %g %g %g %g %g %g %g %g %g %g %g\n",
          ...
          FineMap->MassWasting, FineMap->MassDeposition,
          FineMap->SedimentToChannel, SedMap->Erosion, SedMap->SedFluxOut,
          SedimentOverlandInflow, Network->Erosion, SedMap->RoadSed,
          SedimentOverroadInflow);
}
