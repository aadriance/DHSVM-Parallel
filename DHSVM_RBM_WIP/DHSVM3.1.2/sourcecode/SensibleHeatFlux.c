...
void SensibleHeatFlux(int y, int x, int Dt, float Ra, float ZRef,
  ...
  LocalSoil->TSurf =
      RootBrent(y, x, MinTSurf, MaxTSurf, SurfaceEnergyBalance, Dt, Ra, ZRef,
                ...
                LocalMet->AirDens, LocalMet->Lv, ETot, KhEff, SoilType->Ch[0],
                SoilType->Porosity[0], LocalSoil->Moist[0], FluxDepth,
                LocalMet->Tair, TSoilUpper, TSoilLower, OldTSurf, MeltEnergy);
