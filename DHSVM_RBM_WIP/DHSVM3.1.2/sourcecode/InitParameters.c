/*
 * SUMMARY:      InitParameters.c - Initialize constants for sediment tranport
 * model USAGE:        Part of DHSVM
 *
...
void InitParameters(LISTPTR Input, OPTIONSTRUCT *Options, MAPSIZE *Map,
                    ...
                    TOPOPIX **TopoMap, TIMESTRUCT *Time, float *SedDiams) {
  int i, x, y; /* counter */
  const char *Routine = "InitParameters";
  STRINIENTRY StrEnv[] = {{"SEDOPTIONS", "MASS WASTING", "", ""},
...
/*******************************************************************************
  ...
  Purpose      : Read in the dates to run the mass wasting model. This
information is in the [SEDTIME] section of the sediment input file
...
void InitMassWaste(LISTPTR Input, TIMESTRUCT *Time) {
  ...
  if (Time->NMWMTotalSteps > 0) {
    ...
    if ((*Time).Start.Julian > (*Time).MWM[0].Julian)
      ReportError(
          "First Mass Wasting Date is before the beginning of the model run",
          51);
