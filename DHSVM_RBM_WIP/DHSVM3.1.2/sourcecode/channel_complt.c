...
int channel_save_outflow_text_cplmt(TIMESTRUCT *Time, char *tstring,
  ...
  int err = 0;
  int Dt;
  FILE *out, *out2, *out4, *out3, *out5, *out9, *out10, *out11, *out13, *out14,
      *out6, *out7;
  ...
  /* print the start date and end date. Note that the true start date is a time
  step behind the user specified date when the model outputs data */
  if (flag == 1) {
  ...
  if ((Time->Current.JDay >= Time->Start.JDay + 1) ||
    ...
    if (fprintf(out2, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_inflow: write error:%s",
                    strerror(errno));
      err++;
    }
    ...
    if (fprintf(out4, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_ISW: write error:%s",
                    strerror(errno));
      err++;
    }
    if (fprintf(out5, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_ILW: write error:%s",
                    strerror(errno));
      err++;
    }
    if (fprintf(out9, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR,
                    "channel_save_ActualVaporPressure: write error:%s",
                    strerror(errno));
      err++;
    }
    if (fprintf(out10, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_Wind: write error:%s",
                    strerror(errno));
      err++;
    }
    if (fprintf(out11, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_AirTemp: write error:%s",
                    strerror(errno));
      err++;
    }
    if (fprintf(out13, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_NetLW: write error:%s",
                    strerror(errno));
      err++;
    }
    if (fprintf(out14, "%15s ", tstring) == EOF) {
      error_handler(ERRHDL_ERROR, "channel_save_NetSW: write error:%s",
                    strerror(errno));
      err++;
    }
