...
/*****************************************************************************
  ...
  Sources:
   Smith, R.E., D.C. Goodrich, and C.L. Unkrich, Simulation of selected events
on the Catsop catchment by KINEROS, 1999, A report for the GCTE conference on
  ...
  These two papers and the code that can be downloaded from
www.tucson.ars.ag.gov/kineros have a different formulation of the equation for
hydraulic erosion. The equation used here, is eh = cg(Cmx-Cs)A. cg = vs/h*CH
    ...
    for (x = 0; x < Map->NX; x++) {
      if (INBASIN(TopoMap[y][x].Mask)) {
        if (channel_grid_has_channel(ChannelData->road_map, x, y)) {
          ...
          beta = 3. / 5.;
          alpha = pow(Network[y][x].RoadClass->friction_road *
                          pow((double)dx, 2. / 3.) / sqrt(slope),
                      beta);
          ...
          while (Before(&(VariableTime.Current), &(NextTime.Current))) {
            ...
            for (i = 0; i < CELLFACTOR; i++) {
              ...
              /*Update surface water storage.  Make sure calculated outflow
                ...
                water storage. */
              if (outflow >
                  (Network[y][x].h[i] * dy * dx) / (float)Time->Dt + Runon[i]) {
                outflow =
                    (Network[y][x].h[i] * dy * dx) / (float)Time->Dt + Runon[i];
              }

              /* Accounting for rounding errors */
              check = Network[y][x].h[i] +
                      ((Runon[i] - outflow) * VariableDT / (dy * dx));
              ...
              /*************************************************************/
              /* Don't perform sediment calculations if the road is paved, there
                 is no outflow or if the flow depth is less than the median
                 particle size */
              SedOut = 0.;
              ...
              if ((Network[y][x].RoadClass->erodibility_coeff < 999999) &&
                ...
                /* PrecipMap[y][x].RainFall/Time->Dt is the rainfall intensity
                 * (m/s)*/
                ...
                if (streampower > 0.0004) {
                  Cmx = 0.05 /
                        (DS * pow((PARTDENSITY / WATER_DENSITY - 1.), 2.)) *
                        pow((slope * h / G), 0.5) * (streampower - 0.0004);
                  ...
                  SedOut =
                      (SedIn[i] * (term1 * Runon[i] -
                                   term2 * pow((double)Runon[i], beta)) +
                       Network[y][x].OldSedOut[i] *
                           (term2 * pow((double)Network[y][x].startRunoff[i],
                                        beta) -
                            term3 * Network[y][x].startRunoff[i]) +
                       Network[y][x].OldSedIn[i] *
                           (term2 *
                                pow((double)Network[y][x].startRunon[i], beta) +
                            term3 * Network[y][x].startRunon[i]) +
                       ES + (cg * Cmx * alpha * pow(outflow, beta))) /
                      (term2 * pow(outflow, beta) + term1 * outflow +
                       cg * alpha * pow(outflow, beta));

                  if (SedOut >= Cmx) /* then deposition */
                    SedOut = Cmx;

                  if ((Cmx > 1) || (SedIn[i] > 1) || (SedOut > 1)) {
                    printf("RouteRoad: Invalid results Cmx(%e) SedIn(%e) "
                           "SedOut(%e)\n",
                           Cmx, SedIn[i], SedOut);
                    printf(
                        "DS %f slope %f h %e outflow %e dx %f streampow %e\n",
              ...
              Network[y][x].Erosion +=
                  (((SedIn[i] * Runon[i] - SedOut * outflow) * VariableDT) /
                   (Map->DX * Map->DY)) *
                  (cells / (float)CELLFACTOR);

              /* Save sub-timestep runoff for q(i)(t-1) and q(i-1)(t-1) of next
               * time step. */
              ...
              if (outflow > 0.) {
                ...
                /* If last pixel send outflow off road */
                  ...
                  sedbin = 0;
                  if (Network[y][x].RoadClass->d50_road >
                      SedDiams[NSEDSIZES - 1])
                    sedbin = NSEDSIZES - 1;
                  else {
                  ...
                  if (Network[y][x].RoadClass->crown == CHAN_OUTSLOPED) {
                    SoilMap[y][x].IExcess +=
                        ((outflow * VariableDT) / (Map->DX * Map->DY)) *
                        (cells / (float)CELLFACTOR);
                  ...
                  /* If the road is crowned, then the same amount of outflow
                     goes to the ditch and off the road edge into the same
                     pixel. This is similar to culvert flow. 0.5 accounts for
                     1/2 the cells on are one side of the crown. */
                  else if (Network[y][x].RoadClass->crown == CHAN_CROWNED) {
                    channel_grid_inc_inflow(
                        ChannelData->road_map, x, y,
                        ((outflow * VariableDT) / (Map->DX * Map->DY)) * 0.5 *
                            (cells / (float)CELLFACTOR));
                    ...
                    ChannelData->road_map[x][y]
                        ->channel->sediment.overroadinflow[sedbin] +=
                        SedOut * outflow * VariableDT * PARTDENSITY * 0.5 *
                        (cells / (float)CELLFACTOR);

                    SedMap[y][x].RoadSed += ((SedOut * outflow * VariableDT) /
                                             (Map->DX * Map->DY)) *
                                            0.5 * (cells / (float)CELLFACTOR);
                    ...
                    ChannelData->road_map[x][y]
                        ->channel->sediment.overroadinflow[sedbin] +=
                        SedOut * outflow * VariableDT * PARTDENSITY *
                        (cells / (float)CELLFACTOR);
                  }
                }
              }
...
float FindDTRoad(ROADSTRUCT **Network, TIMESTRUCT *Time, int y, int x, float dx,
