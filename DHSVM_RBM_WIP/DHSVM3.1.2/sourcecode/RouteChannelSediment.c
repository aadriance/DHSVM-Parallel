...
int InitChannelSediment(Channel *Head, AGGREGATED *Total) {
  if (Head != NULL) {
    ...
    int i;
    float InitialDepth =
        0.010; /* initial depth of sediment in the channel, m */
    float bulkporosity, initvol;

    bulkporosity = 0.245 + 0.14 * pow((double)DEBRISd50,
                                      -0.21); /* Komura, 1961 relation */
    ...
    while (Current) {
      ...
      for (i = 0; i < NSEDSIZES; i++) {
        Current->sediment.mass[i] = initvol * (1. - bulkporosity) *
                                    ((float)PARTDENSITY) *
                                    (1. / ((float)NSEDSIZES));
...
*****************************************************************************/
void RouteChannelSediment(Channel *Head, TIMESTRUCT Time, DUMPSTRUCT *Dump,
  ...
  float Qup, Qdown;
  float phi = 0.55, theta = 0.55, term3,
        term4; /*space and time weighting factors*/
  int i, tstep;
  int order;
  int order_count;
      ...
      if (Current->order == order) {
        ...
        if (Qavg > 0) {
          if (Current->slope > 0.0) {
            flowdepth = pow(Qavg * Current->class2->friction /
                                (Current->class2->width * sqrt(Current->slope)),
                            0.6);
            V = Qavg / (flowdepth * Current->class2->width);
          } else
            V = 0.01;
          if (Current->length / V < minDT_sed)
            minDT_sed = 1.0 * Current->length / V;
          ...
          for (i = 0; i < NSEDSIZES; i++) {
            ...
            Current->sediment.inflowrate[i] =
                Current->sediment.inflow[i] / (float)Time.Dt;
          }
          ...
          for (tstep = 0; tstep < numinc; tstep++) {
            ...
            /****************************************/
            /*DO NOT USE BAGNOLD's EQ. FOR D<0.015 mm - this is wash load
             * anyway*/
            for (i = 0; i < NSEDSIZES; i++) {
              ...
              /* Set theta to 1.0 to prevent instabilities during mass wasting
               * inflow  */
              if (Current->sediment.debrisinflow[i] > 0)
                theta = 1.0;

              /*  Set theta to 1.0 to prevent instabilities during large
              ...
              while (abs(mass_error) > 0.1) {
                ...
                if (SedDiams[i] <
                    0.062) { /* per Wicks and Bathurst, wash load */
                  TotalCapacity = Current->sediment.inflowrate[i] +
                                  Current->sediment.mass[i] / DT_sed;
                } else {
                  TotalCapacityUp =
                      CalcBagnold(DS, &Time, Qup, Current->class2->width,
                                  Current->class2->friction, Current->slope);
                  TotalCapacityDown =
                      CalcBagnold(DS, &Time, Qdown, Current->class2->width,
                                  Current->class2->friction, Current->slope);
                  TotalCapacity =
                      phi * TotalCapacityDown + (1.0 - phi) * TotalCapacityUp;
                  TotalCapacity -=
                      CapacityUsed; /* Avoid mult use of streampower */
                }
                ...
                if (Current->sediment.outflowrate[i] >= TotalCapacity) {
                  ...
                  mass_error = (lateral_sed_inflow_rate +
                  ...
                  if (abs(mass_error) > 0.1) {
                    ...
                    mass_error = (lateral_sed_inflow_rate +
                                  ...
                                  Current->sediment.outflowrate[i]) *
                                 DT_sed;
                    dMdt = lateral_sed_inflow_rate +
                           Current->sediment.inflowrate[i] -
                           Current->sediment.outflowrate[i];
                  }
                }
                mass_error =
                    (lateral_sed_inflow_rate + Current->sediment.inflowrate[i] -
                     dMdt - Current->sediment.outflowrate[i]) *
                    DT_sed;
                error_count++;
              ...
              if (error_count > 2) {
                printf("Warning: Unable to reduce mass error below specified "
          ...
          for (i = 0; i < NSEDSIZES; i++) {
            ...
            if (Current->outlet != NULL) {
              Current->outlet->sediment.inflow[i] +=
                  Current->sediment.last_outflow[i];
            ...
            /* If no stream segment outlet, there is a road sink or a basin
               outlet. Track this for the sediment mass balance. */
            else {
            ...
            Current->sediment.outflowconc +=
                1000.0 * Current->sediment.outflow[i] / Current->outflow;
          }
        }      /* end 	if(Qavg > 0){ */
        else { /* if Qvag < 0 */
          for (i = 0; i < NSEDSIZES; i++) {
        ...
        /* the next 7 lines are from channel_route_network -- closes the loop
         * above */
        order_count += 1;
...
*****************************************************************************/
void RouteCulvertSediment(CHANNEL *ChannelData, MAPSIZE *Map, TOPOPIX **TopoMap,
                          SEDPIX **SedMap, AGGREGATED *Total, float *SedDiams) {
  int x, y;
      ...
      if (INBASIN(TopoMap[y][x].Mask)) {
        ...
        for (i = 0; i < NSEDSIZES; i++) {
          ...
          if (channel_grid_has_channel(ChannelData->stream_map, x, y)) {
            /* Percent delivery to streams is conservative and based on particle
             * size */
            if (SedDiams[i] <= 0.063) {
              ChannelData->stream_map[x][y]
                  ->channel->sediment.overlandinflow[i] += CulvertSedFlow;
              Total->CulvertSedToChannel += CulvertSedFlow;
              CulvertSedFlow = 0.;
            }
            if ((SedDiams[i] > 0.063) && (SedDiams[i] <= 0.5)) {
              ChannelData->stream_map[x][y]
                  ->channel->sediment.overlandinflow[i] += 0.3 * CulvertSedFlow;
            ...
            if ((SedDiams[i] > 0.5) && (SedDiams[i] <= 2.)) {
              ChannelData->stream_map[x][y]
                  ->channel->sediment.overlandinflow[i] += 0.1 * CulvertSedFlow;
