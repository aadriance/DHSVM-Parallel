 ...
 * COMMENTS:
                 This program is considerably changed to fix the problems
 including: 1) runoff from some basins cell is rounted to the neighnoring cells
                                    that are outside of basin boundary
                                 2) unfilled sinks due to the D8 and D4
 algorithm difference between ArcGIS and DHSVM. Main changes are made to
 slope_aspect(), flow_fractions() & ElevationSlopeAspect()
...
int xdirection[NDIRS] = {0, 1, 0, -1};
int ydirection[NDIRS] = {-1, 0, 1, 0};

float temp_aspect[NNEIGHBORS] = {225., 180., 135., 90., 45., 0., 315., 270.};

/* NNEIGHBORS used for redistribution of subsurface flow in topoindex
 * and for slope/aspect calculations, must equal 8. */
int xneighbor[NNEIGHBORS] = {-1, 0, 1, 1, 1, 0, -1, -1};

int yneighbor[NNEIGHBORS] = {1, 1, 1, 0, -1, -1, -1, 0};
...
/* -------------------------------------------------------------
   ...
   ------------------------------------------------------------- */
static void slope_aspect(float dx, float dy, float celev,
                         float nelev[NNEIGHBORS], float *slope, float *aspect) {
  int n;
  float dzdx, dzdy;
  float *dummyelev;
  ...
  dzdx = ((dummyelev[0] + 2 * dummyelev[7] + dummyelev[6]) -
          (dummyelev[2] + 2 * dummyelev[3] + dummyelev[4])) /
         (8 * dx);
  dzdy = ((dummyelev[0] + 2 * dummyelev[1] + dummyelev[2]) -
          (dummyelev[4] + 2 * dummyelev[5] + dummyelev[6])) /
         (8 * dy);
...
void ElevationSlopeAspect(MAPSIZE *Map, TOPOPIX **TopoMap) {
      ...
      if (INBASIN(TopoMap[y][x].Mask)) {
        ...
        for (n = 0; n < NNEIGHBORS; n++) {
          ...
          if (valid_cell(Map, xn, yn)) {
            neighbor_elev[n] = ((TopoMap[yn][xn].Mask) ? TopoMap[yn][xn].Dem
                                                       : (float)OUTSIDEBASIN);
          } else {
        ...
        flow_fractions(Map->DX, Map->DY, TopoMap[y][x].Slope,
                       TopoMap[y][x].Aspect, neighbor_elev,
  ...
  /* End of modifications to create ordered cell coordinates.  SRW 10/02, LCB
   * 03/03 */
  return;
}
...
void quick(ITEM *OrderedCells, int count) { qs(OrderedCells, 0, count - 1); }
...
**********************************************************************/
{
  ...
  do {
    while (item[i].Rank < x.Rank && i < right)
      i++;
    while (x.Rank < item[j].Rank && j > left)
      j--;
...
float ElevationSlope(MAPSIZE *Map, TOPOPIX **TopoMap, FINEPIX ***FineMap, int y,
  ...
  for (n = 0; n < NNEIGHBORS; n++) {
    ...
    if (valid_cell_fine(Map, xn, yn)) {
      ...
      // (equivalent to checking whether parent coarse grid cell is within
      // coarse mask)
      if (INBASIN(TopoMap[coarsej][coarsei].Mask)) {

        bedrock_elev[n] =
            (((*FineMap[yn][xn]).Mask) ? (*FineMap[yn][xn]).bedrock
                                       : (float)OUTSIDEBASIN);
        soil_elev[n] =
            (((*FineMap[yn][xn]).Mask)
                 ? (*FineMap[yn][xn]).bedrock + (*FineMap[yn][xn]).sediment
                 : (float)OUTSIDEBASIN);
      }
    }
  }
  /*  Find bedrock slope in all directions. Negative slope = ascent, positive
   * slope = descent.  */
  dx = Map->DMASS;
  dy = Map->DMASS;
  ...
  for (n = 0; n < NNEIGHBORS; n++) {
    ...
    if (n == 0 || n == 2 || n == 4 || n == 6)
      temp_slope[n] =
          (atan((celev - bedrock_elev[n]) / length_diagonal)) * DEGPRAD;
    else if (n == 1 || n == 5)
  ...
  /* Find largest (positive) slope, this is the direction of failure along
     bedrock plain. Backtracking isn't a problem if using the bedrock, but sinks
     may exist. */
  ...
  *Aspect = -99.;
  ...
  if (direction == 0 || direction == 2 || direction == 4 || direction == 6)
    Slope = (atan((celev - soil_elev[direction]) / length_diagonal)) * DEGPRAD;
  ...
  // if(Slope < 0.0 ) {
  // fprintf(stderr, "Sink encountered in cell y= %d x= %d, all routes from here
  // go up!\n", y,x);
  // }
