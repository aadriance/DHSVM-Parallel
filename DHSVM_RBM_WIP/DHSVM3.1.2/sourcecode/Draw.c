...
void draw(DATE *Day, int first, int DayStep, MAPSIZE *Map, int NGraphics,
          ...
          FINEPIX ***FineMap, VEGPIX **VegMap, TOPOPIX **TopoMap,
          PRECIPPIX **PrecipMap, float **PrismMap, float **SkyViewMap,
          unsigned char ***ShadowMap, EVAPPIX **EvapMap, RADCLASSPIX **RadMap,
          MET_MAP_PIX **MetMap, ROADSTRUCT **Network,
          OPTIONSTRUCT *Options) { /*begin */
  ...
  int buf = 50;
  int sample =
      1; /* if sample =0, then average if compression needed, otherwise if =1
            then sample */
      ...
      for (k = 0; k < NGraphics; k++) {
        ...
        if (MapNumber < 3 || MapNumber > 6 || draw_static_colorbar == 1) {
          ...
          if (expand > 0) {
            ...
            PY = PY * (Map->NY * expand + buf) +
                 20; /*top 20 pixels reserved for date stamp */
          } else {
              ...
              for (j = 0; j < Map->NY; j++) {
                if (INBASIN(TopoMap[j][i].Mask)) {
                  if (SnowMap[j][i].Swq > MAX_SURFACE_SWE) {
                    ...
                    temp = 2.10e3 * (SnowMap[j][i].TSurf * surf_swe +
            ...
            for (i = 0; i < Map->NX; i++) {
              for (j = 0; j < Map->NY; j++) {
                if (INBASIN(TopoMap[j][i].Mask)) {
                  ...
                  temp = SoilMap[j][i].Moist[0] /
                         SType[SoilMap[j][i].Soil - 1].Porosity[0] * 100.0;
                  if (temp > max)
                    max = temp;
                  if (temp < min)
                    min = temp;
            ...
            for (i = 0; i < Map->NX; i++) {
              for (j = 0; j < Map->NY; j++) {
                if (INBASIN(TopoMap[j][i].Mask)) {
                  ...
                  temp = SoilMap[j][i].Moist[1] /
                         SType[SoilMap[j][i].Soil - 1].Porosity[1] * 100.0;
                  if (temp > max)
                    max = temp;
                  if (temp < min)
                    min = temp;
          ...
          if (expand > 0) {
            ...
            for (i = 0; i < Map->NX / (-expand); i++) {
              for (j = 0; j < Map->NY / (-expand); j++) {
                ...
                /* for map numbers of 50 or larger, which are the channel and
                 * runoff */
                /* subsurface interception, get the max for each aggregated
                 * pixel */
                if (MapNumber > 49) {
                  max_temp = -10000.0;
                  for (ie = 0; ie < (-expand); ie++) {
          ...
          if (draw_static_colorbar == 1) {
            ...
            for (j = 0; j < Map->NY * re; j++) {
              ...
              /*    if((int)((float)(j*50/(Map->NY*re))/scale+min)==0)
               * XSetForeground(display,gc,white); */
