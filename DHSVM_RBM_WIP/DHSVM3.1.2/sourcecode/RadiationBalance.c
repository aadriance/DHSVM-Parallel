...
layers
  ...
  Required     :
    ...
    float Rs           - Incoming shortwave radiation adjusted by topo shading
        ...
        float VIC_Rs       - Input 'observed' shortwave radiation in the forcing
file float Ld           - Incoming longwave radiation (W/m2) float Tair
- Ambient air temperature (C) float Tcanopy      - Canopy temperature from the
previous timestep (C) float Tsoil        - soil surface temperature from the
previous timestep (C) int NAct           - Number of vegetation layers above the
snow VEGTABLE VType     - Information about number of veg layers SNOWPIX
LocalSnow  - Information about snow conditions at current pixel PIXRAD *LocalRad
- Components of radiation balance for current pixel
...
void RadiationBalance(OPTIONSTRUCT *Options, int HeatFluxOption,
  ...
  if (Options->StreamTemp) {
    ...
    // total shortwave radiation without any shading effect even if shading is
    // on
...
void LongwaveBalance(OPTIONSTRUCT *Options, unsigned char OverStory, float F,
  ...
  LocalRad->PixelLongIn = Ld;
  if (OverStory == TRUE) {
    LocalRad->PixelLongOut =
        LocalRad->LongOut[0] * F + LocalRad->LongOut[1] * (1 - F);
  } else {
...
void ShortwaveBalance(OPTIONSTRUCT *Options, unsigned char OverStory, float F,
                      float Rs, float Rsb, float Rsd, float Tau, float *Albedo,
                      PIXRAD *LocalRad) {
  ...
  if (Options->StreamTemp && !Options->CanopyShading) {
    if (OverStory == TRUE) {
      ...
      LocalRad->PixelBeam =
          Rsb * (1 - F) + Rsb * Tau * F; // direct beam radiation
      LocalRad->PixelDiffuse =
          LocalRad->RBMNetShort - LocalRad->PixelBeam; // diffuse radiation
    } else {
