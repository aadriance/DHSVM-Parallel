 ...
 * COMMENTS: Modified by Ted Bohn on 10/1/2013
             ...
             Introduced a new parameter, DEPTH_THRESHOLD.  When water table
 depth is shallower than this threshold, transmissivity decays exponentially
                         with depth as before.  When water table depth is deeper
 than this threshold, transmissivity decays linearly until it reaches 0.
 DEPTH_THRESHOLD must be specified in the config file.
...
float CalcTransmissivity(float SoilDepth, float WaterTable, float LateralKs,
  ...
  if (fequal(KsExponent, 0.0))
    ...
    /* a smaller value of WaterTable variables indicates a higher actual water
     * table depth */
    if (WaterTable < DepthThresh) {
      Transmissivity =
          (LateralKs / KsExponent) *
          (exp(-KsExponent * WaterTable) - exp(-KsExponent * SoilDepth));
    } else {
      TransThresh = (LateralKs / KsExponent) * (exp(-KsExponent * DepthThresh) -
                                                exp(-KsExponent * SoilDepth));
      ...
      Transmissivity =
          (SoilDepth - WaterTable) / (SoilDepth - DepthThresh) * TransThresh;
    }
  }
