...
*****************************************************************************/
PIXMET
MakeLocalMetData(int y, int x, MAPSIZE *Map, int DayStep, OPTIONSTRUCT *Options,
                 int NStats, METLOCATION *Stat, uchar *MetWeights,
                 float LocalElev, RADCLASSPIX *RadMap, PRECIPPIX *PrecipMap,
                 MAPSIZE *Radar, RADARPIX **RadarMap, float **PrismMap,
                 SNOWPIX *LocalSnow, SNOWTABLE *SnowAlbedo, float ***MM5Input,
                 float ***WindModel, float **PrecipLapseMap,
  ...
  if (Options->MM5 == TRUE) {
    ...
    for (i = 0; i < NStats; i++) {
      ...
      LocalMet.Tair +=
          CurrentWeight * LapseT(Stat[i].Data.Tair, Stat[i].Elev, LocalElev,
  ...
  /* is not all that hard, but is prone to user error (e.g. GMT time shifts,
   * etc) */
  ...
  /* a finer resolution than this would require a higher min angle or more
   * memory */
  ...
  if (Options->Shading == TRUE) {
    ...
    /* if canopy shading is computed, then the skyview factor will be compared
   with riparian canopy openess */
    ...
    if (Options->PrecipType == STATION && Options->Prism == FALSE) {
      ...
      for (i = 0; i < NStats; i++) {
        ...
        if (Options->PrecipLapse == MAP)
          ...
          PrecipMap->Precip +=
              CurrentWeight * LapsePrecip(Stat[i].Data.Precip, Stat[i].Elev,
                                          LocalElev, Stat[i].Data.PrecipLapse);
      }
      ...
      for (i = 0; i < NStats; i++) {
        ...
        if (Options->Outside == FALSE)
          PrecipMap->Precip += CurrentWeight * Stat[i].Data.Precip /
                               PrismMap[Stat[i].Loc.N][Stat[i].Loc.E] *
                               PrismMap[y][x];
        else
          PrecipMap->Precip += CurrentWeight * Stat[i].Data.Precip /
  ...
  /* interpolation scheme to handle the mess of different formats of met
   * stations */
  ...
  if (LocalSnow->HasSnow) {
    if (PrecipMap->SnowFall > 0.0)
      ...
      LocalSnow->LastSnow++;
    LocalSnow->Albedo =
        CalcSnowAlbedo(LocalSnow->TSurf, LocalSnow->LastSnow, SnowAlbedo);
  } else
    LocalSnow->LastSnow = 0;
