 ...
 * DESCRIPTION:  For lateral sediment inflow, finds the particle diameters for
 each portion.  Assumes a lognormal distribution
 * DESCRIP-END.
...
#ifndef NORMALDIST
#define NORMALDIST(mean, stdev, y)                                             \
  (4.91 * stdev * (pow(y, .14) - pow((1 - y), .14)) + mean)
#endif
  ...
  mn = log10(DEBRISd50);
  std = log10(DEBRISd90) -
        log10(DEBRISd50) / (NORMALDIST(0, 1, 0.9) - NORMALDIST(0, 1, 0.5));
