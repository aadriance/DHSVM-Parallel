...
void InitNewMonth(TIMESTRUCT *Time, OPTIONSTRUCT *Options, MAPSIZE *Map,
  ...
  if (Options->Prism == TRUE) {
    ...
    sprintf(FileName, "%s.%02d.%s", Options->PrismDataPath, Time->Current.Month,
    ...
    flag = Read2DMatrix(FileName, Array, NumberType, Map->NY, Map->NX, 0,
                        VarName, 0);
  ...
  if (Options->Shading == TRUE) {
    ...
    if (!(Array1 = (unsigned char *)calloc(Map->NY * Map->NX,
                                           sizeof(unsigned char))))
      ReportError((char *)Routine, 1);
    for (i = 0; i < Time->NDaySteps; i++) {
      Read2DMatrix(FileName, Array1, NumberType, Map->NY, Map->NX, i, VarName,
                   i);
      for (y = 0; y < Map->NY; y++) {
        for (x = 0; x < Map->NX; x++) {
  ...
  for (i = 0; i < NVegs; i++) {
    for (j = 0; j < VType[i].NVegLayers; j++) {
      ...
      VType[i].MaxInt[j] =
          VType[i].LAI[j] * VType[i].Fract[j] * LAI_WATER_MULTIPLIER;
...
void InitNewStep(INPUTFILES *InFiles, MAPSIZE *Map, TIMESTRUCT *Time,
   ...
   * Time->Current.Year,Time->Current.Month, Time->Current.Day,
   * Time->Current.Hour);*/
  ...
  SolarHour(
      SolarGeo->Latitude, (Time->DayStep + 1) * ((float)Time->Dt) / SECPHOUR,
      ((float)Time->Dt) / SECPHOUR, SolarGeo->NoonHour, SolarGeo->Declination,
      SolarGeo->Sunrise, SolarGeo->Sunset, SolarGeo->TimeAdjustment,
      SolarGeo->SunEarthDistance, &(SolarGeo->SineSolarAltitude),
      &(SolarGeo->DayLight), &(SolarGeo->SolarTimeStep), &(SolarGeo->SunMax),
  ...
  if (Options->MM5 == TRUE) {
    ...
    Read2DMatrix(InFiles->MM5Temp, Array, NumberType, MM5Map->NY, MM5Map->NX,
                 Step);
    for (y = 0; y < Map->NY; y++)
      for (x = 0; x < Map->NX; x++) {
    ...
    Read2DMatrix(InFiles->MM5Wind, Array, NumberType, MM5Map->NY, MM5Map->NX,
                 Step);
    for (y = 0; y < Map->NY; y++)
      for (x = 0; x < Map->NX; x++) {
        MM5Y = (int)((y + MM5Map->OffsetY) * Map->DY / MM5Map->DY);
    ...
    Read2DMatrix(InFiles->MM5Terrain, Array, NumberType, MM5Map->NY, MM5Map->NX,
                 Step);
    for (y = 0; y < Map->NY; y++)
      for (x = 0; x < Map->NX; x++) {
        MM5Y = (int)((y + MM5Map->OffsetY) * Map->DY / MM5Map->DY);
    ...
    Read2DMatrix(InFiles->MM5Lapse, Array, NumberType, MM5Map->NY, MM5Map->NX,
                 Step);
    for (y = 0; y < Map->NY; y++)
      for (x = 0; x < Map->NX; x++) {
        MM5Y = (int)((y + MM5Map->OffsetY) * Map->DY / MM5Map->DY);
