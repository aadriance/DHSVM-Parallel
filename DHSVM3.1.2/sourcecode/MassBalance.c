...
void MassBalance(DATE *Current, FILES *Out, FILES *SedOut, AGGREGATED *Total,
  ...
  NewWaterStorage = Total->Soil.IExcess + Total->Road.IExcess +
                    Total->CanopyWater + Total->SoilWater + Total->Snow.Swq +
                    Total->Soil.SatFlow + Total->Soil.DetentionStorage;
...
%.2e  %5.2f  %5.2f  %7.4f  %7.4f  %7.4f  %6.3f  %.2e  %5.2f  %.2e  %7.3f \n",
          ...
          Total->Snow.Swq, Total->Soil.SatFlow, Total->ChannelInt,
          Total->RoadInt, Total->CulvertReturnFlow, Total->Evap.ETot,
          Total->Precip.Precip, Total->Snow.VaporMassFlux,
          Total->Snow.CanopyVaporMassFlux, Mass->OldWaterStorage,
          Total->CulvertToChannel, Total->RunoffToChannel, MassError);
  ...
  if (Options->Sediment) {
    ...
    fprintf(SedOut->FilePtr, " %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g \n",
            ...
            Total->Road.Erosion, Total->Sediment.RoadSed, Total->DebrisInflow,
            Total->SedimentOverlandInflow, Total->SedimentOverroadInflow,
            Total->SedimentOutflow, Total->CulvertReturnSedFlow,
            Total->CulvertSedToChannel, Mass->LastChannelSedimentStorage,
            SedMassError);
  }
}
...
/*       15. Total channel erosion mass balance error for the current time step
 * (kg) */
