...
#define DEPTHTHRESHOLD                                                         \
  0.0001 /* Min. depth below which no transport occurs                         \
...
*****************************************************************************/
float CalcBagnold(float DS, TIMESTRUCT *Time, float outflow, float width,
                  float n, float slope) {
  ...
  settling =
      sqrt(36 * visc * visc / (DS * DS) +
           0.667 * (PARTDENSITY - WATER_DENSITY) * G * DS / WATER_DENSITY) -
      6 * visc / DS;
  ...
  if (flowdepth < DEPTHTHRESHOLD)
    ...
    /*printf ( "DS= %.5f   Vs= %.5f  Dt= %d  n= %.3f flow=
      %.3f",DS,settling,Time->Dt ,n,outflow); printf(" depth= %.3f V=
      %.3f\n",flowdepth,V); */
    ...
    if (DS / MMTOM <= 0.6) {
      ...
      tanalphamax =
          (0.85 - 0.29 * DS / MMTOM > 0.75) ? 0.75 : 0.85 - 0.29 * DS / MMTOM;
      tanalpha = A * log10(taustar) + B;
      tanalpha = (tanalpha > tanalphamax) ? tanalphamax : tanalpha;
    } else {
      tanalpha = 0.375;
