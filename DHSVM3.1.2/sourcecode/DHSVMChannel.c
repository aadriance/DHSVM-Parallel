...
void InitChannel(LISTPTR Input, MAPSIZE *Map, int deltat, CHANNEL *channel,
  ...
  int i;
  STRINIENTRY StrEnv[] = {{"ROUTING", "STREAM NETWORK FILE", "", ""},
  ...
  if (strncmp(StrEnv[stream_class].VarStr, "none", 4)) {
    ...
    if ((channel->stream_class = channel_read_classes(
             StrEnv[stream_class].VarStr, stream_class, FALSE)) == NULL) {
      ReportError(StrEnv[stream_class].VarStr, 5);
    }
    if ((channel->streams = channel_read_network(StrEnv[stream_network].VarStr,
                                                 channel->stream_class,
                                                 MaxStreamID)) == NULL) {
      ReportError(StrEnv[stream_network].VarStr, 5);
    }
    if ((channel->stream_map = channel_grid_read_map(
             channel->streams, StrEnv[stream_map].VarStr, SoilMap)) == NULL) {
  ...
  if (strncmp(StrEnv[road_class].VarStr, "none", 4)) {
    ...
    if ((channel->road_map = channel_grid_read_map(
             channel->roads, StrEnv[road_map].VarStr, SoilMap)) == NULL) {
...
void RouteChannel(CHANNEL *ChannelData, TIMESTRUCT *Time, MAPSIZE *Map,
  ...
  for (y = 0; y < Map->NY; y++) {
    for (x = 0; x < Map->NX; x++) {
      if (INBASIN(TopoMap[y][x].Mask)) {
        ...
        if (channel_grid_has_channel(ChannelData->road_map, x, y) &&
            !channel_grid_has_sink(ChannelData->road_map, x,
                                   y)) { /* road w/o sink */
  ...
  if (ChannelData->roads != NULL) {
    ...
    channel_save_outflow_text(buffer, ChannelData->roads, ChannelData->roadout,
                              ChannelData->roadflowout, flag);
  }
  ...
  if (ChannelData->streams != NULL) {
    ...
    if (Options->StreamTemp)
      channel_save_outflow_text_cplmt(Time, buffer, ChannelData->streams,
                                      ChannelData, flag);
  }
}
