 ...
 * DESCRIPTION:  Calculate the shadow length resulting from riparian vegetation
 zone
 * DESCRIP-END.
 * FUNCTIONS:
 * Modification
...
void CalcCanopyShading(Channel *Channel, SOLARGEOMETRY *SolarGeo) {
  ...
  while (Channel) {
    ...
    if (SolarAltitude > 0) {
      ...
      Dx1 = HDEM * fabs(sin(SolarGeo->SolarAzimuth - StreamAzimuth) /
                        tan(SolarAltitude)) -
            (CanopyBankDist + Channel->class2->width);
      Dx2 = HDEM * fabs(sin(SolarGeo->SolarAzimuth - StreamAzimuth) /
                        tan(SolarAltitude)) -
            CanopyBankDist;
      ...
      if (ShadeCase > 1)
        ...
        Net_Shade_Fctr = CalcShadeDensity(
            ShadeCase, HDEM, Channel->class2->width, SolarGeo->SolarAzimuth,
            StreamAzimuth, SolarAltitude, TREEHEIGHT, BUFFERWIDTH, Dx1, Dx2,
            Extn);
      else
        Net_Shade_Fctr = 0.;
      if (Net_Shade_Fctr > 1) {
    ...
    Channel->NLW = Channel->NLW * MIN(Channel->skyview, SKOP) +
                   0.96 * (1 - MIN(Channel->skyview, SKOP)) * 0.96 * STEFAN *
                       pow((double)(Channel->ATP + 273.15), 4);
...
float CalcShadeDensity(int ShadeCase, float HDEM, float WStream,
  ...
  case 3:
    Not_Shaded = WStream - Dx2;
    Shaded = Dx2;
    Pavg = 0.5 * BUFFERWIDTH / cos(SunAltitude) /
           fabs(sin(SunAzimuth - StreamAzim));
    break;
  case 4:
    Pavg = 0.5 * (Dx1 + Dx2) / cos(SunAltitude) /
           fabs(sin(SunAzimuth - StreamAzim));
    break;
  case 5:
    Pavg = 0.5 * (Dx1 + BUFFERWIDTH) / cos(SunAltitude) /
           fabs(sin(SunAzimuth - StreamAzim));
    break;
  case 6:
...
/*****************************************************************************
  ...
  Purpose:	 This subroutine calculates the skyview factor above the
riparaian vegetation
******************************************************************************/
