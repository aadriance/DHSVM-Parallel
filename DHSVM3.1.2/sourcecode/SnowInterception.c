...
*****************************************************************************/
void SnowInterception(int y, int x, int Dt, float F, float LAI, float MaxInt,
                      float MaxSnowIntCap, float MDRatio, float SnowIntEff,
                      float Ra, float AirDens, float EactAir, float Lv,
                      PIXRAD *LocalRad, float Press, float Tair, float Vpd,
                      float Wind, float *RainFall, float *SnowFall,
  ...
  *IntSnow += DeltaSnowInt;
  ...
  if ((*IntRain + *RainFall) <= MaxWaterInt) {
    ...
    *IntRain += *RainFall;
    ...
    /* pixel depth */
    RainThroughFall =
        (*IntRain + *RainFall - MaxWaterInt) * F + (*RainFall * (1 - F));
    /* physical depth */
    *IntRain = MaxWaterInt;
  }
  ...
  /* Calculate the vapor mass flux between the canopy and the surrounding
     air mass - snow covered aerodynamic resistance is assumed to increase by an
     order of magnitude based on Lunderg et al 1998, Journal of Hydrological
     Processes */
  ...
  *SnowFall = SnowThroughFall + ReleasedMass;
  ...
  if (Understory)
    ...
    *MomentSq = pow(LD_FallVelocity * WATER_DENSITY, 2) * PI / 6 *
    ...
    /* If no understory, part of the rainfall reaches the ground as direct
     * throughfall. */
    *MomentSq = pow(LD_FallVelocity * WATER_DENSITY, 2) * PI / 6 *
                    pow(LEAF_DRIP_DIA, 3) * Drip / Dt +
                (1 - F) * MS_Rainfall;
}
