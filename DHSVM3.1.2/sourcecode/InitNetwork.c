...
void InitNetwork(int NY, int NX, float DX, float DY, TOPOPIX **TopoMap,
                 ...
                 ROADSTRUCT ***Network, CHANNEL *ChannelData, LAYER Veg,
      ...
      for (x = 0; x < NX; x++) {
        if (INBASIN(TopoMap[y][x].Mask)) {
          ...
          AdjustStorage(
              ...
              VType[VegMap[y][x].Veg - 1].RootDepth, (*Network)[y][x].Area, DX,
              DY, (*Network)[y][x].BankHeight, (*Network)[y][x].PercArea,
              (*Network)[y][x].Adjust, &((*Network)[y][x].CutBankZone));
          ...
          if (channel_grid_has_channel(ChannelData->road_map, x, y)) {
            ...
            (*Network)[y][x].FlowLength = channel_grid_flowlength(
                ChannelData->road_map, x, y, (*Network)[y][x].FlowSlope);
            (*Network)[y][x].RoadArea =
                channel_grid_cell_width(ChannelData->road_map, x, y) *
  ...
  if (doimpervious) {
    if (!(inputfile = fopen(Options->ImperviousFilePath, "rt"))) {
      fprintf(stderr, "User has specified a percentage impervious area \n");
