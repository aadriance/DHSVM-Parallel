 ...
 * Modification: Changes are made to exclude the impervious channel cell (with
                 a non-zero impervious fraction) from surface routing. In the
 original code, some impervious channel cells are routed to themselves causing
                                 overestimated runoff in those cells (Ning,
 2013).
          ...
          if (INBASIN(TopoMap[y][x].Mask)) {
            if (!channel_grid_has_channel(ChannelData->stream_map, x, y)) {
              if (VType[VegMap[y][x].Veg - 1].ImpervFrac > 0.0) {
                /* Calculate the outflow from impervious portion of urban cell
                 * straight to nearest channel cell */
                SoilMap[TopoMap[y][x].drains_y][TopoMap[y][x].drains_x]
                    .IExcess +=
                    (1 - VType[VegMap[y][x].Veg - 1].DetentionFrac) *
                    VType[VegMap[y][x].Veg - 1].ImpervFrac *
                    SoilMap[y][x].Runoff;
                /* Retained water in detention storage */
                SoilMap[y][x].DetentionIn =
                    VType[VegMap[y][x].Veg - 1].DetentionFrac *
                    VType[VegMap[y][x].Veg - 1].ImpervFrac *
                    SoilMap[y][x].Runoff;
                ...
                SoilMap[y][x].DetentionOut =
                    SoilMap[y][x].DetentionStorage *
                    VType[VegMap[y][x].Veg - 1].DetentionDecay;
                SoilMap[TopoMap[y][x].drains_y][TopoMap[y][x].drains_x]
                    .IExcess += SoilMap[y][x].DetentionOut;
                ...
                for (n = 0; n < NDIRS; n++) {
                  ...
                  if (valid_cell(Map, xn, yn)) {
                    SoilMap[yn][xn].IExcess +=
                        (1 - VType[VegMap[y][x].Veg - 1].ImpervFrac) *
                        SoilMap[y][x].Runoff *
                        ((float)TopoMap[y][x].Dir[n] /
            ...
            } else if (channel_grid_has_channel(ChannelData->stream_map, x,
                                                y)) {
    ...
    /***********************************************************************************************************************/
      ...
      while (Before(&(VariableTime.Current), &(NextTime.Current))) {
        ...
        for (k = (Map->NumCells) - 1; k > -1; k--) {
          ...
          if (channel_grid_has_channel(ChannelData->stream_map, x, y) ||
            ...
            if (Runon[y][x] > 0.0001 || outflow > 0.0001) {
              sedoutflow = ((VariableDT / Map->DX) * Runon[y][x] +
                            alpha * beta * outflow *
                                pow((outflow + Runon[y][x]) / 2.0, beta - 1.) +
                            (SoilMap[y][x].IExcessSed) * Map->DX * VariableDT /
                                Time->Dt) /
                           ((VariableDT / Map->DX) +
                            alpha * beta *
                                pow((outflow + Runon[y][x]) / 2.0, beta - 1.));
              ...
              sedoutflow =
                  SoilMap[y][x].IExcessSed * Map->DX * Map->DY / Time->Dt;
            else
              sedoutflow = 0.0;
            if (sedoutflow < 0.0)
              sedoutflow = 0.0;
            ...
            if (sedoutflow >
              ...
              sedoutflow =
                  SoilMap[y][x].IExcessSed * (Map->DX * Map->DY) / Time->Dt +
                  (Runon[y][x]);
            SoilMap[y][x].IExcessSed +=
                (Runon[y][x] - sedoutflow) * VariableDT / (Map->DX * Map->DY);
          }
          /*Make sure calculated outflow doesn't exceed available water, and
           * update surface water storage */
          if (outflow >
              (SoilMap[y][x].IExcess * (Map->DX * Map->DY) / Time->Dt +
               Runon[y][x]))
            outflow = SoilMap[y][x].IExcess * (Map->DX * Map->DY) / Time->Dt +
                      (Runon[y][x]);

          SoilMap[y][x].IExcess +=
              (Runon[y][x] - outflow) * VariableDT / (Map->DX * Map->DY);
          ...
          SedOut = 0.;
          if (Options->SurfaceErosion) {
            /* Only perform sediment routing if there is outflow, and and for
              ...
              if ((h > 0.001) && (streampower > SETTLECRIT)) {
                ...
                SedOut =
                    (SedIn[y][x] * (term1 * Runon[y][x] -
                                    term2 * pow((double)Runon[y][x], beta)) +
                     SedMap[y][x].OldSedOut *
                         (term2 * pow((double)SoilMap[y][x].startRunoff, beta) -
                          term3 * SoilMap[y][x].startRunoff) +
                     SedMap[y][x].OldSedIn *
                         (term2 * pow((double)SoilMap[y][x].startRunon, beta) +
                          term3 * SoilMap[y][x].startRunon) +
                     DR + floweff * Map->DY * vs * TC) /
                    (term2 * pow(sedoutflow, beta) + term1 * sedoutflow +
                     floweff * Map->DY * vs);
                ...
                SedMap[y][x].SedFluxOut +=
                    (SedOut * sedoutflow *
                     VariableDT); /* total sediment (m3) */
                SedMap[y][x].Erosion +=
                    (SedIn[y][x] * Runon[y][x] - SedOut * sedoutflow) *
                    VariableDT / (Map->DX * Map->DY) *
                    1000.; /* total depth of erosion (mm) */
          ...
          /* Save sub-timestep runoff for q(i)(t-1) and q(i-1)(t-1) of next time
           * step. */
          SoilMap[y][x].startRunoff = sedoutflow;
          SoilMap[y][x].startRunon = Runon[y][x];

          /******************************* Calculate total runoff in m/dhsvm
           * timestep*************************************************/
          /* This is serving the purpose of holding this value for Cournat
          condition calculation. It does not trully repreent the Runoff, because
          if there is a channel there is no outflow. Instead, IExcess is updated
          based on Runon in the same manner of the original DHSVM */
          SoilMap[y][x].Runoff += sedoutflow * VariableDT / (Map->DX * Map->DY);

          /* Sediment from pixels with channels goes into the channel.  This
           * assumes that all surface erosion is of the smallest particle sizes
           * (first size, index 0) */
          /* Note that stream_map and road_map are indexed by [x][y], unlike the
           * other "map"-type variables. */
          ...
          if ((Options->SurfaceErosion) && (SedOut > 0.)) {
            ...
            if (channel_grid_has_channel(ChannelData->stream_map, x, y)) {
              ...
              ChannelData->stream_map[x][y]
                  ->channel->sediment.overlandinflow[sedbin] +=
...
/*****************************************************************************
  FindDT()
  Find the variable time step that will satisfy the courant condition for
stability in overland flow routing.
*****************************************************************************/
float FindDT(SOILPIX **SoilMap, MAPSIZE *Map, TIMESTRUCT *Time,
             TOPOPIX **TopoMap, SOILTABLE *SType) {
  int x, y;
  /* JSL: slope is manning's slope; alpha is channel parameter including wetted
     perimeter, manning's n, and manning's slope.  Beta is 3/5 */
  float slope;
  double beta, alpha;
  double Ck;
  float DT, minDT;
      ...
      if (INBASIN(TopoMap[y][x].Mask)) {
        if (SoilMap[y][x].Runoff > 0.0) {
          ...
          alpha = pow((double)SType[SoilMap[y][x].Soil - 1].Manning *
                          pow((double)Map->DX, (double)(2. / 3.)) / sqrt(slope),
                      (double)beta);

          /* Calculate flow velocity from discharge  using manning's equation.
           */
          Ck = 1. /
               (alpha * beta * pow((double)SoilMap[y][x].Runoff, beta - 1.));
...
void SedimentFlag(OPTIONSTRUCT *Options, TIMESTRUCT *Time) {
  ...
  if ((Options->Sediment) && (Options->ErosionPeriod)) {
    if ((Time->Current.Julian >= StartSed) &&
        (Time->Current.Julian <= EndSed)) {
