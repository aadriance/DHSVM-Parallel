  ...
  ***************************************************************************/
  ...
  for (k = (Map->NumCellsfine) - 1; k > -1; k--) {
    ...
    for (n = 0; n < NNEIGHBORS; n++) {
      ...
      if (valid_cell_fine(Map, xn, yn)) {
        ...
        if (INBASIN(TopoMap[coarsei][coarsej].Mask)) {
          ...
          // Solve for all grid cells within the coarse mask, not just the fine
          // mask.
          neighbor_elev[n] =
              ((TopoMap[coarsei][coarsej].Mask) ? (*FineMap[yn][xn]).Dem
                                                : (float)OUTSIDEBASIN);
        }
      }
    }
    ...
    case 8:
      ...
      if (lower == 8) {
        /* if this is a flat area then tanbeta = sum of (0.5 * vertical
           resolution of elevation data)/ honizontal distance between centers of
           neighboring grid cells */
        tanbeta[y][x] =
            ((NNEIGHBORS / 2) * ((0.5 * VERTRES) / length_diagonal)) +
