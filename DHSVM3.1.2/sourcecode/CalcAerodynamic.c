...
/*****************************************************************************
  ...
  Comments     :
*****************************************************************************/
void CalcAerodynamic(int NVegLayers, unsigned char OverStory, float n,
                     float *Height, float Trunk, float *U, float *U2mSnow,
                     float *Ra, float *RaSnow) {
  float d_Lower;
  float d_Upper;
  float K2;
  float Uh;
  ...
  /* Overstory present, one or two vegetation layers possible */
    ...
    if (Zt > (2. + Z0_Lower + d_Lower)) {
      ...
      Ra[1] = log((2. + Z0_Lower) / Z0_Lower) * log((Zt - d_Lower) / Z0_Lower) /
              (K2 * Ut);
    }
    ...
    /* case 2: the wind profile to a height of 2m above the lower boundary
      ...
      Ra[1] = log((Zt - d_Lower) / Z0_Lower) * log((Zt - d_Lower) / Z0_Lower) /
                  (K2 * Ut) +
              Height[0] * log((Zref - d_Upper) / Z0_Upper) /
                  (n * K2 * (Zw - d_Upper)) *
                  (exp(n * (1 - Zt / Height[0])) -
    ...
    /* case 3: the top of the overstory is less than 2 m above the lower
      ...
      fprintf(stderr, "WARNING:  Top of overstory is less than 2 meters above "
                      "the lower boundary\n");
    }
...
/*****************************************************************************
  ...
  gcc -DTEST_MAIN -o test_aero ReportError.c InitErrorMessage.c
CalcAerodynamic.c -lm
