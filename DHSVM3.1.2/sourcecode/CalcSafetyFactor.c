...
float CalcSafetyFactor(float Slope, int Soil, float SoilDepth, int Veg,
                       ...
                       SOILTABLE *SType, float Swq, float Depth, int iter) {
  ...
  if (Slope >= 0.) {
    ...
    /*     if(SnowDensity>0) printf("Surcharge w/  snow %4.3f SnowDensity
     * %4.3f\n",  */
    /* 			     Surcharge, SnowDensity);  */
    ...
    fc_soil_density =
        SType[Soil - 1].Dens[0] + (SType[Soil - 1].FCap[0] * WATER_DENSITY);
    ...
    term1 = ((soil_cohes_kg + root_cohes_kg) /
             ((Surcharge + SoilDepth * fc_soil_density) * cos(slope_angle_rad) *
              cos(slope_angle_rad))) +
    ...
    if (term1 <= tan(slope_angle_rad)) {
      ...
      safetyfactor =
          (((2. * (soil_cohes_kg + root_cohes_kg)) /
            (WATER_DENSITY * SoilDepth * (sin(2. * slope_angle_rad)))) +
           ((loading - M) *
            ((tan(angle_int_frict_rad)) / (tan(slope_angle_rad))))) /
          loading;
    }
