...
void MainMWM(SEDPIX **SedMap, FINEPIX ***FineMap, VEGTABLE *VType,
  ...
  float *
      *InitialSediment; /* Place holder of pixel sediment load at beginning of
                           time step. */
  float
      SedToDownslope;   /* Sediment wasted from a pixel, awaiting redistribution
                         */
  ...
  ****************************************************************************/
  ...
  /* Redistribute soil moisture from coarse grid to fine grid. The is done
     similarly to Burton, A. and J.C. Bathurst, 1998, Physically based modelling
     of shallow landslide sediment yield as a catchment scale, Environmental
     Geology, 35 (2-3), 89-99.*/
  ...
  for (i = 0; i < Map->NY; i++) {
    for (j = 0; j < Map->NX; j++) {
      ...
      if (INBASIN(TopoMap[i][j].Mask)) {
        ...
        /* TopoIndexAve is the TopoIndex for the coarse grid calculated as the
           average of the TopoIndex of the fine grids in the coarse grid. */
          ...
          for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
            ...
            if (SoilMap[i][j].Depth > SoilMap[i][j].TableDepth) {
              ...
              if (FineMapTableDepth < 0.) {
                ...
                (*FineMap[y][x]).SatThickness =
                    (*FineMap[y][x]).sediment - FineMapTableDepth;
            } else
              (*FineMap[y][x]).SatThickness = 0.;

            FineMapSatThickness += (*FineMap[y][x]).SatThickness;

            if ((ii == Map->DY / Map->DMASS - 1) &
                (jj == Map->DX / Map->DMASS - 1)) {

              /* Calculating the difference between the volume of water
                 distributed (only saturated) and available volume of water (m3)
               */
              Redistribute[i][j] =
                  (Map->DY * Map->DX * (SoilMap[i][j].Depth - TableDepth)) -
  ...
  *****************************************************************************/
  ...
  if (!(InitialSegmentSedimentm =
            (float **)calloc(MaxStreamID, sizeof(float *))))
    ReportError("MainMWM", 1);
  for (i = 1; i < MaxStreamID + 1; i++) {
    if (!(InitialSegmentSedimentm[i] =
              (float *)calloc(NSEDSIZES, sizeof(float))))
      ReportError("MainMWM", 1);
  }
  ...
  for (iter = 0; iter < massitertemp; iter++) {
          ...
          for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
            for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
              ...
              if (INBASIN((*FineMap[y][x]).Mask)) {
                ...
                if ((*FineMap[y][x]).SatThickness / SoilMap[i][j].Depth >
                        MTHRESH &&
                    failure[y][x] == 0 && (*FineMap[y][x]).sediment > 0.0) {

                  LocalSlope =
                      ElevationSlope(Map, TopoMap, FineMap, y, x, &nexty,
                                     &nextx, y, x, &SlopeAspect);

                  if (LocalSlope >= 10.) {
                    factor_safety = CalcSafetyFactor(
                        LocalSlope, SoilMap[i][j].Soil,
                        (*FineMap[y][x]).sediment, VegMap[i][j].Veg, SedType,
                        VType, (*FineMap[y][x]).SatThickness, SType,
                        SnowMap[i][j].Swq, SnowMap[i][j].Depth, iter);
                    ...
                    if (factor_safety < FS_CRITERIA && factor_safety > 0) {
                      ...
                      /* Update sediment depth. All sediment leaves failed fine
                       * pixel */
                      ...
                      /* Follow failures down slope; skipped if no original
                       * failure. */
                      while (failure[y][x] == 1 && checksink == 0 &&
                             !channel_grid_has_channel(ChannelData->stream_map,
                                                       coursej, coursei) &&
                        ...
                        if (!INBASIN(TopoMap[coursei][coursej].Mask)) {
                          ...
                          printf("WARNING: attempt to propagate failure to "
                                 "grid cell outside basin: y %d x %d\n",
                                 y, x);
                          printf("Depositing wasted sediment in grid cell y %d "
                                 "x %d\n",
                                 prevy, prevx);
                          (*FineMap[prevy][prevx]).sediment += SedFromUpslope;
                          SedFromUpslope = SedToDownslope = 0.0;

                          // Since we're returning SedFromUpslope to the upslope
                          // pixel, the upslope pixel can't be considered as
                          // part of the failure
                          ...
                          LocalSlope = ElevationSlope(Map, TopoMap, FineMap, y,
                                                      x, &nexty, &nextx, prevy,
                                                      prevx, &SlopeAspect);
                          ...
                          if (LocalSlope >= 0.) {
                            ...
                            factor_safety = CalcSafetyFactor(
                                LocalSlope, SoilMap[coursei][coursej].Soil,
                            ...
                            /* check if fine pixel fails */
                            if (factor_safety < FS_CRITERIA &&
                                factor_safety > 0) {
                              numpixels += 1;
                              failure[y][x] = 1;

                              /* Update sediment depth. All sediment leaves
                               * failed fine pixel */
                              SedToDownslope = (*FineMap[y][x]).sediment;
                              (*FineMap[y][x]).sediment = 0.0;
                              ...
                              // Remove the sediment we added for the slope
                              // calculation and instead prepare to distribute
                              // this sediment along runout zone
                              (*FineMap[y][x]).sediment -= SedFromUpslope;
                            }
                          } /* end  if(LocalSlope >= 0) { */
                          else {
                      ...
                      // y and x are now the coords of the first pixel of the
                      // runout (downslope neighbor of final pixel of the
                      // failure); this is the pixel that caused the last loop
                      // to exit, whether due to being a sink, not failing,
                      // being in a coarse pixel containing a stream,
                      // or being outside the basin

                      // If current cell is outside the basin,
                      // stop processing this runout and go to next failure
                      // candidate
                      if (!INBASIN(TopoMap[coursei][coursej].Mask)) {
                        continue;
                      }
                      ...
                      while (LocalSlope > 4. &&
                        ...
                        if (INBASIN(TopoMap[coursei][coursej].Mask)) {
                          ...
                          printf("WARNING: attempt to propagate runout to grid "
                                 "cell outside the basin: y %d x %d\n",
                                 y, x);
                          printf(
                              "Final grid cell of runout will be: y %d x %d\n",
                              prevy, prevx);
                        }
                      }
                      prevy = y;
                      ...
                      for (count = 0; count < cells; count++) {
                        ...
                        // If this node has a channel, then this MUST be the end
                        // of the queue
                        if (channel_grid_has_channel(ChannelData->stream_map,
                                                     coursej, coursei)) {
                          /* TotalVolume at this point is a depth in m over one
                             fine map grid cell - convert to m3 */
                          SedimentToChannel = TotalVolume *
                                              (Map->DMASS * Map->DMASS) /
                                              (float)cells;
                        } else {
                          /* Redistribute sediment equally among all hillslope
                           * cells. */
                      ...
                      if (SedimentToChannel > 0.0) {
                        if (SlopeAspect < 0.) {
                          ...
                          // Add Current value of SedimentToChannel to running
                          // total for this FineMap cell (allowing for more than
                          // one debris flow to end at the same channel)
                          (*FineMap[y][x]).SedimentToChannel +=
                              SedimentToChannel;

                          // Now route SedimentToChannel through stream network
                          RouteDebrisFlow(&SedimentToChannel, coursei, coursej,
                                          SlopeAspect, ChannelData, Map);
                        }
                      }
        ...
        if (INBASIN(TopoMap[i][j].Mask)) {
          ...
          for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
            for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
               ...
               */
              /* If not in stochastic mode, then allow a history of past
                 failures and do not reset sediment depth */
              if (massitertemp > 1) {
    ...
    update_sediment_mass(ChannelData->streams, SegmentSedimentm, massitertemp);
      ...
      if (INBASIN(TopoMap[i][j].Mask)) {
        ...
        for (ii = 0; ii < Map->DY / Map->DMASS; ii++) {
          for (jj = 0; jj < Map->DX / Map->DMASS; jj++) {
            ...
            (*FineMap[y][x]).DeltaDepth =
                (*FineMap[y][x]).sediment - SoilMap[i][j].Depth;
          }
        }
      }
    }
  ...
  for (i = 1; i < MaxStreamID + 1; i++) {
    ...
    if (SegmentSediment[i] < 0.0)
      SegmentSediment[i] = 0.0;
  }
  ...
  fprintf(fs, "%-20s %.4f %.4f %7d\n", buffer, avgnumfailures, avgpixperfailure,
          numlikelyfailedpixels);
  printf("%.4f failures; %.4f pixels per failure; %d pixels have failure "
         "likelihood > %.2f\n",
         avgnumfailures, avgpixperfailure, numlikelyfailedpixels,
         failure_threshold);
  fclose(fs);
